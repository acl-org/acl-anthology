...
[`bin/extract_pdf_metadata.py`](../bin/extract_pdf_metadata.py),
...
The map uses an offline, committed-data workflow. The website build never calls
a geocoding service:

1. `bin/geocode_affiliations.py` reads affiliation strings from the Anthology's
  `<author>` metadata and matches normalized names against a downloaded ROR
  data dump. The ROR ID is the canonical organization identity; ROR also
  supplies the organization name, country, and sector.
2. When ROR links an organization to an unambiguous Wikidata entity, the script
  uses that entity's `P625` coordinate as the institution location. Otherwise,
  it retains the ROR GeoNames locality centroid as a fallback.
3. Results are written to the generated
  `data/geo/affiliation_geocache.json`. Each resolved entry records its
  `coordinate_source` as `wikidata` or `ror-geonames`; unresolved strings are
  cached as `null`. Do not edit this file manually.
4. Corrections belong in `data/geo/affiliation_overrides.json`. The website
  build loads these entries after the generated cache, so they take precedence.
5. `export_affiliation_map()` aggregates by ROR ID, never by coordinates, and
  counts an organization at most once per paper. Coordinates are presentation
  metadata: multiple organizations may legitimately share a fallback city
  centroid. The map labels those fallbacks as approximate city locations.

Regenerate the cache after ingesting new affiliation data:
...
- ROR (organization identity, metadata, and locality fallbacks): <https://ror.org>
- Wikidata (institution coordinates linked from ROR): <https://www.wikidata.org>
