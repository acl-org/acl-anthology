...
- **Joint venue tag**: add the secondary `<venue>` to the ingested volume's
  ...
  merged venue file.
- **New venue name**: open `data/yaml/venues/<slug>.yaml` and strip ordinals/years.
...
| `could not find paper ID … watermarked_pdfs` | misnamed/misplaced PDF dir | symlink `watermarked_pdfs` (step 1) |
...
| `'bibkey' must match regex … ('-etal-2026-…' )` on a LATER ingest | an already-committed volume has a paper with an **empty author surname** → bibkey starts with `-`; that volume now fails to load, blocking every subsequent ingest | fix the offending author (give the surname, or split first/last) in that volume's `papers.yml` and **re-ingest that volume**, then resume the rest |
