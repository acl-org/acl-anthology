...
def best_split(name):
    ...
    for i in range(1,len(words)):	# at least one word in each part
        ...
        first_probs = [ log((first_count[x]+0.01)/first_total)
                        for x in words[0:i] ]
        first_score = max ( log((first_full_count[first]+0.000001)/first_total),
                            sum(first_probs) )
        last_probs = [ log((last_count[x]+0.01)/last_total)
                       for x in words[i:] ]
