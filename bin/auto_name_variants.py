                ...
                for jj in range(len(y[j])+1):
                    if ii > 0 and jj > 0:
                        if x[i][ii-1] == y[j][jj-1]:
                            d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i,ii-1,j,jj-1]) # no change
                        else:
                            d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i,ii-1,j,jj-1] + c_subst)
                    if ii > 0:
                        d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i,ii-1,j,jj] + c_delete)
                    if jj > 0:
                        d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i,ii,j,jj-1] + c_insert)

                    if i > 0 and ii == 0 and j > 0 and jj == 0:
                        d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i-1,len(x[i-1]),j-1,len(y[j-1])]) # no change (space)
                        for kk in range(1, len(x[i-1])):
                            d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i-1,kk,j-1,len(y[j-1])] + c_delete_suffix)
                            
                    if i > 0 and ii == 0:
                        d[i,ii,j,jj] = min(d[i,ii,j,jj], d[i-1,0,j,jj] + c_delete_word)
    ...
    for filename in sys.argv[1:]:
        ...
        for paper in volume.findall('paper'):
            for person in paper.xpath('./author|./editor'):
                ...
                counts[first, last] += 1
                papers[first, last].append('{}-{}'.format(volume.attrib['id'], paper.attrib['id']))
