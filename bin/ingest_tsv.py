...
def main(args):
    ...
    for row in csv.DictReader(args.meta_file, delimiter="\t"):
        ...
        if current_collection_id == collection_id:
            if row["Completed"] == "FALSE":
                if args.force:
                    print("Warning: Conference {collection_id} is not marked as completed.", file=sys.stderr)
                else:
                    print("Warning: Conference {collection_id} is not marked as completed and no -f, can't ingest.", file=sys.stderr)
                    sys.exit(1)
    ...
    for paperid, row in enumerate(csv.DictReader(args.tsv_file, delimiter='\t'), start_id):
        ...
        if "Pagenumbers" in row:
            pages = row["Pagenumbers"]
            make_simple_element("pages", pages, parent=paper)

        url = f"{collection_id}-{volume_id}.{paperid}"
        pdf_local_path = os.path.join(args.anthology_files_path, collection_id, f"{url}.pdf")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--anthology-files-path', default=f"{os.environ.get('HOME')}/anthology-files/pdf",
