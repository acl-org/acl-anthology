...
def export_conference_volume(volume, xml_body)
    ...
    event_metadata.add_element "conference_date", 
...
=begin
Assign DOI for conference/workshop/journal papers published by ACL since 2012.
P: ACL, D: EMNLP, E: EACL, N: NAACL, S: SemEval/Sem, W: Workshops by ACL, Q: TACL

Note: 
1. Papers from non-ACL publishers will be automatically skipped.
2. Papers from workshops specified in the parameter will be skipped.
...
namespace :export do
  desc "Export each volume to a single doi"
	task :doi_single, [:name, :email, :proceeding, :ignored_workshops] => :environment do |t, args|
		unless args.name or args.email or args.proceeding
			abort("PLease pass depositor's name and email, and the proceeding and ignored workshops as parameter!")
		end
		...
		if proceeding.start_with?("W") # If we have a workshop, the volume series will have 2 digits
		...
		volume_series.each do |v|
			@volume = Volume.find_by_anthology_id(proceeding + "-" + v.to_s)
			# Note: publisher is missing for some volumes in database.
			if @volume and @volume.publisher and @volume.publisher.include?('Association for Computational Linguistics') and !ignored_workshops.include?(@volume.anthology_id)
				...
				xml_file = File.new("export/doi/" + proceeding + "-" + v.to_s + ".xml",'w:UTF-8')
